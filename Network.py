...
def extract_nodes_and_edges(
    ...
    :param gdf_roads: 路网数据，必须包含 'type' 和几何列（LineString），geopandas.GeoDataFrame类型
    :param speed_map: 道路类型（type字段） -> 速度 (km/h)，dict[int, float]类型
    :return: (node_coords, edges, coord_to_id)
    """
    if speed_map is None:
        speed_map = {1: 100, 2: 60, 3: 40, 4: 30, 5: 15}

    node_coords: list[Coord] = []
    coord_to_id: dict[Coord, NID] = {}
    edges: list[Edge] = []

    def get_impedance(length_m: float, road_type: int) -> float:
        speed_kmh = speed_map.get(road_type, 20)
        speed_ms = speed_kmh * 1000 / 3600  # m/s
        return length_m / speed_ms  # time in seconds
    ...
    for _, row in gdf_roads.iterrows():
        ...
        u = coord_to_id[start]
        v = coord_to_id[end]
        weight = get_impedance(row['length'], int(row['type']))

        edges.append((u, v, weight))
        edges.append((v, u, weight))  # bidirectional

    return node_coords, edges, coord_to_id

def extract_nodes_and_edges_by_distance(
        gdf_roads: gpd.GeoDataFrame,
        speed_map: Optional[dict[int, float]] = None  # 保留参数但不再使用
) -> tuple[list[Coord], list[Edge], dict[Coord, NID]]:
    """
    从路网 GeoDataFrame 提取节点、边和坐标到 ID 的映射。
    边的权重现在为几何长度（单位：米），即按距离计算。
    ...
    def get_weight_by_distance(length_m: float) -> float:
        return length_m  # 直接使用长度作为权重（单位：米）
    ...
    for _, row in gdf_roads.iterrows():
        ...
        u = coord_to_id[start]
        v = coord_to_id[end]
        weight = get_weight_by_distance(row['length'])  # 改为距离权重
...
def a_star_pathfinding(
    ...
    avg_speed_mps: float = 36.0
) -> tuple[float, list[NID], int]:
    """
    A* 算法（时间最优），使用欧氏距离 / 平均速度作为启发函数。
    """
    def heuristic_time(n1: NID, n2: NID) -> float:
        x1, y1 = node_coords[n1]
        x2, y2 = node_coords[n2]
        dx = x2 - x1
        dy = y2 - y1
        euclid = (dx * dx + dy * dy) ** 0.5
        return euclid / avg_speed_mps
    ...
    f_score[start] = heuristic_time(start, goal)
    ...
    while open_set:
        ...
        for neighbor, time_cost in graph[current]:
            ...
            if tentative_g < g_score[neighbor]:
                ...
                f_score[neighbor] = tentative_g + heuristic_time(neighbor, goal)
